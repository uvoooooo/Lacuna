...
输入一段典型的单方面社媒发帖（内置 `--demo` 文本）：

> 我在喜多多集团勤勤恳恳工作了六年，带头研发了核心系统。上周五，部门经理突然叫我去办公室，跟我说我被开除了，连补偿金都没提。他踢走老员工，霸占我的期权，这是赤裸裸的压榨！

Lacuna 的审计发现（节选）。「被开除」事件对齐到本体后，那些必然存在却从未出现的角色就是结论：

| | 缺失要素 | 值得追问的问题 |
|---|---|---|
| 1 | 开除理由 `必要` | 对方给出的开除理由是什么？ |
| 2 | 正规解雇流程 `预期` | 解除劳动合同走了哪些正规流程（书面通知、HR 面谈、离职交接）？ |
| 3 | 事先警告/绩效记录 `预期` | 此前是否有警告、绩效沟通或处分记录？ |
| 4 | 当事人回应/申诉 `预期` | 当事人是否申诉、仲裁或有其他回应？ |

其中两个空缺最扎眼。像喜多多这样的科技大厂，辞退必然伴随正规 HR 流程（书面通知、HR 面谈、离职交接），而帖子描述的是经理在办公室里当场开除，对流程只字未提。另外，帖子指控的是明确的违法行为（无补偿辞退、侵占期权），却完全没提劳动仲裁或任何法律行动。如果指控属实，法律救济是最自然的下一步，它的缺席本身就是信号。

除空缺之外：主观段落（「他踢走老员工，霸占我的期权，这是赤裸裸的压榨！」）被标为主观/情绪化而非事实；「在先劳动关系」作为 `inferred` 节点补入图谱（被开除必然蕴含它，尽管原帖从未提及）。不判断谁对谁错：只呈现说了什么、暗示了什么、漏了什么。

复现：

```bash
uv run python -m narrative_audit --demo                    # 终端报告
uv run python -m narrative_audit --card demo.html --demo   # 可分享卡片
```
